<template>
	<div :id="side">
		...
		<b-container class="bv-example-row" v-if="!loading">
			...
			<b-row>
				<b-col cols="8" md="3" lg="3" v-for="(people,index) in allPeople" 
					...
					<div class="card box">
					  	...
					  	<div class="card-block">
					    	<h4 class="card-title" style="text-align:center;">{{people.name}}</h4>
					    	<ul class="card-text">
								<li>Gender: {{ people.gender }}</li>
								<li>Height: {{ people.height }}</li>
								<li>Mass: {{ people.mass }}</li>
					    	</ul>
					    	<router-link :to="{name: 'onePeople', params: {id: getId(people.url)}}">
		    	...
		    	<b-col>
		    		<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-center" 
							style="box-shadow: rgba(0, 0, 0, 0.4);">
						   	<li class="page-item">
	...
	export default{
		...
		methods: {
			...
			getPeople(index) {
				this.loading = true;
				this.initializer();
				this.$http.get('https://swapi.co/api/people/?page=' + index)
						  .then((response) => {
			...
			setImage(people, index){
				this.$http.get('https://www.googleapis.com/customsearch/v1?key=AIzaSyAI-2lr6p-qas_iY-jwcjsb3BEpIxQK7ik&cx=007669208564501026839%3Art6dk6nxzxq&q='+people.name) 
					.then((response) => {
			...
			setAllImages(){
				this.allPeople.forEach((people, index)=>{
					this.setImage(people, index);
				});
			}
		},
		mounted() {
