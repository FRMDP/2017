<template>
	<div :id="side">
		...
		<b-container class="bv-example-row" v-if="!loading">
			...
			<b-row>
				<b-col cols="8" md="3" lg="3" v-for="(planet,index) in planets" 
					...
					<div class="card box">
					  	...
					  	<div class="card-block">
					    	<h4 class="card-title" style="text-align:center;">{{planet.name}}</h4>
					    	<ul class="card-text">
		    		...
		    		<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-center">
						   	...
						   	<li v-for="page in pages" class="page-item">
						   		<a class="page-link"
						   			 @click="getPlanets(page)">
						   			{{ page }}
						   		</a>
						   	</li>
						   	<li class="page-item">
						      	<a class="page-link" aria-label="Next"
						      		@click="getPlanets(numberOfPages)">
	...
	export default{
		...
		methods: {
	    	...
	    	getPlanets(index) {
	    		this.loading = true;
		    	this.initializer();
	    		this.$http.get('https://swapi.co/api/planets/?page=' + index)
		        	.then(response => {
			...
			setImage(planet, index){
				this.$http.get('https://www.googleapis.com/customsearch/v1?key=AIzaSyAI-2lr6p-qas_iY-jwcjsb3BEpIxQK7ik&cx=007669208564501026839%3Art6dk6nxzxq&q='+planet.name) 
					.then((response) => {
