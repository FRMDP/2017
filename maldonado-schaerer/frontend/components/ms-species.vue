<template>
	<div :id="side">
		...
		<b-container class="bv-example-row" v-if="!loading">
			...
			<b-row>
				<b-col cols="8" md="3" lg="3" v-for="(specie,index) in species" 
					...
					<div class="card box">
					  	...
					  	<div class="card-block">
					    	<h4 class="card-title" style="text-align:center;">{{specie.name}}</h4>
					    	<ul class="card-text">
								<li>Classification: {{ specie.classification }}</li>
								<li>Height: {{ specie.average_height }}</li>
								<li>Lifespan: {{ specie.average_lifespan }}</li>
		    		...
		    		<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-center">
						   	...
						   	<li v-for="page in pages" class="page-item">
						   		<a class="page-link"
						   			 @click="getSpecies(page)">
						   			{{ page }}
						   		</a>
						   	</li>
						   	<li class="page-item">
						      	<a class="page-link" aria-label="Next"
						      		@click="getSpecies(numberOfPages)">
	...
	export default{
	    ...
	    methods: {
	    	...
	    	getSpecies(index) {
	    		this.loading = true;
		    	this.initializer();
	    		this.$http.get('https://swapi.co/api/species/?page=' + index)
		        	.then(response => {
			...
			setImage(specie, index){
				this.$http.get('https://www.googleapis.com/customsearch/v1?key=AIzaSyAI-2lr6p-qas_iY-jwcjsb3BEpIxQK7ik&cx=007669208564501026839%3Art6dk6nxzxq&q='+specie.name) 
					.then((response) => {
