<template>
	<div :id="side">
		...
		<b-container class="bv-example-row" v-if="!loading">
			...
			<b-row>
				<b-col cols="8" md="3" lg="3" v-for="(vehicle,index) in vehicles" 
	...
	export default{
		...
		methods: {
	    	...
	    	getVehicles(index) {
	    		this.loading = true;
		    	this.initializer();
	    		this.$http.get('https://swapi.co/api/vehicles/?page=' + index)
		        	.then(response => {
			...
			setImage(vehicle, index){
				this.$http.get('https://www.googleapis.com/customsearch/v1?key=AIzaSyAI-2lr6p-qas_iY-jwcjsb3BEpIxQK7ik&cx=007669208564501026839%3Art6dk6nxzxq&q='+vehicle.name) 
					.then((response) => {
