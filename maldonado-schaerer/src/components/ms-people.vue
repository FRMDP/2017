	...
	<div>
		<div v-if="!error" :id="side">
			...
			<b-container class="bv-example-row" v-if="!loading">
				...
				<b-row>
					<b-col cols="8" md="5" lg="3" v-for="(people,index) in allPeople" :key="people.name"
	...
	export default{
		...
		methods: {
			...
			getPeople(index) {
				this.loading = true;
				this.initializer();
				this.$http.get(urls.baseInfoSearch + 'people/?page=' + index)
						  .then((response) => {
						  		this.error = false;
			...
			setImage(people, index){
				...
				this.$http.get(urls.imageSearch + people.name + vaderAux) 
					.then((response) => {
