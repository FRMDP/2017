	...
	<div>
		<div v-if="!error" :id="side">
			...
			<b-container class="bv-example-row" v-if="!loading">
				...
				<b-row>
					<b-col cols="8" md="5" lg="3" v-for="(planet,index) in planets" :key="planet.name"
	...
	export default{
		...
		methods: {
	    	...
	    	getPlanets(index) {
	    		this.loading = true;
		    	this.initializer();
	    		this.$http.get(urls.baseInfoSearch + 'planets/?page=' + index)
		        	.then(response => {
		        		this.error = false;
			...
			setImage(planet, index){
				this.$http.get(urls.imageSearch + planet.name) 
					.then((response) => {
