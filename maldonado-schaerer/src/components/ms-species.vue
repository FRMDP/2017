	...
	<div>
		<div v-if="!error" :id="side">
			...
			<b-container class="bv-example-row" v-if="!loading">
				...
				<b-row>
					<b-col cols="8" md="5" lg="3" v-for="(specie,index) in species" :key="specie.name"
	...
	export default{
	    ...
	    methods: {
	    	...
	    	getSpecies(index) {
	    		this.loading = true;
		    	this.initializer();
	    		this.$http.get(urls.baseInfoSearch + 'species/?page=' + index)
		        	.then(response => {
		        		this.error = false;
			...
			setImage(specie, index){
				this.$http.get(urls.imageSearch + specie.name) 
					.then((response) => {
