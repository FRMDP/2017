	...
	<div>
		<div v-if="!error" :id="side">
			...
			<b-container class="bv-example-row" v-if="!loading">
				...
				<b-row>
					<b-col cols="8" md="5" lg="3" v-for="(starship,index) in starships" :key="starship.name"
 	...
 	export default{
		...
		methods:{
		    ...
		    getStarships(index) {
		    	this.loading = true;
		    	this.initializer();
		    	this.$http.get(urls.baseInfoSearch + 'starships/?page=' + index)
		        	.then(response => {
		        		this.error = false;
			...
			setImage(starship, index){
				this.$http.get(urls.imageSearch + starship.name) 
					.then((response) => {
